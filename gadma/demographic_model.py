...
############################################################################
# Copyright (c) 2017 Noskova Ekaterina
# All Rights Reserved
...
class Period(object):
    ...
    def __init__(self,
        ...
        sizes_of_populations :  the list of sizes of population AT THE END of the period
        growth_types :  the list of growth types of populations during the period (0 is for sudden change, 1 for linear growth and 2 for exponential)
        ...
        if self.is_first_period:
            ...
            self.number_of_parameters = 1 + self.number_of_populations * 2 + (
                0 if self.migration_rates == None else
    ...
    def mutate_by_parameter(self, param_index, change, sign, N_A, bounds):
        ...
        # otherwise change corresponding parameter
            ...
            self.get_sizes_of_populations()[param_index - 1] = max(
                self.get_sizes_of_populations()[param_index - 1], bounds.min_N * N_A)
            self.get_sizes_of_populations()[param_index - 1] = min(
                self.get_sizes_of_populations()[param_index - 1], bounds.max_N * N_A)
            ...
            if self.migration_rates[x][y] == 0 and sign == 1:
                self.migration_rates[x][y] = np.random.uniform(bounds.min_M / N_A, min(bounds.max_M / size_of_pop, 10.0 / size_of_pop)) 
            else:
                self.migration_rates[x][y]
                self.migration_rates[x][
                    y] *= 1 + sign * change
                self.migration_rates[x][y] = max(self.migration_rates[x][y], bounds.min_M / size_of_pop)
                self.migration_rates[x][y] = min(self.migration_rates[x][y], bounds.min_M / size_of_pop)
    ...
    def __str__(self):
        ...
        '''
        if self.is_first_period:
            return "[ " + list_float_representation([self.get_sizes_of_populations()[0]]) + " ]"
        if self.is_split_of_population:
            return '[ ' + float_representation(100 * self.split_prop) + '%, ' + list_float_representation(
                self.get_sizes_of_populations()) + ' ]'
        return '[ ' + float_representation(self.time) + ', ' + list_float_representation(
            self.get_sizes_of_populations()) + ', ' + str(
                self.growth_types) + ('' if self.migration_rates is None else
                                      (', ' + migr_float_representation(self.migration_rates))) + ' ]'
...
class Split(Period):
    ...
    def __init__(self, split_prop, population_to_split,
        ...
        '''
        self.population_to_split = population_to_split if population_to_split >= 0 else len(
    ...
    def mutate(self, change, sign, min_N, N_A):
        ...
        self.split_prop = min(
            self.split_prop, 
            1 - min_N * N_A / self.sizes_of_pops_before[self.population_to_split])
        self.split_prop = max(
            self.split_prop,
            min_N * N_A / self.sizes_of_pops_before[self.population_to_split])
...
class Demographic_model:
    ...
    def __init__(self, params, structure=None, random=True, restore_string=None):
        '''
        params :    an object with parameters for working
            input_data :    given SFS
            input_file :    filename of given SFS
            initial_structure :  structure of default models (if structure is None)
            final_structure :  structure of final model (for increase_complexity function)
        ...
        structure :    the structure of the model. If it is None, then the structure will be params.initial_structure
        random :    if False, then create empty model, else create random model with required structure
        '''
        self.not_r = False
        self.params = params
    ...
    def init_random_model(self, structure):
        ...
        for i in xrange(structure[0] - 1):
            self.add_period(
                Period(
                    time=np.random.uniform(self.params.min_T, self.params.max_T),
                    sizes_of_populations=[np.random.uniform(self.params.min_N, self.params.max_N)],
                    growth_types=[random.choice([0,1,2])]))
        for num_of_pops, number_of_periods in enumerate(structure[1:]):
            num_of_pops += 2
            self.add_period(
                Split(
                    split_prop=np.random.uniform(self.params.min_N, 1 - self.params.min_N),
        ...
        for i, period in enumerate(self.periods):
            ...
            for p in xrange(period.number_of_populations):
                if i+1 < self.number_of_periods and self.periods[i+1].is_split_of_population and p == self.periods[i+1].population_to_split:
                    period.sizes_of_populations[p] = max(period.sizes_of_populations[p], 2*N_A*self.params.min_N)
                period.sizes_of_populations[p] = max(period.sizes_of_populations[p], N_A*self.params.min_N)
                period.sizes_of_populations[p] = min(period.sizes_of_populations[p], N_A*self.params.max_N)
                ...
                for p2 in period.populations():
                    if p == p2:
                        continue
                    period.migration_rates[p][p2] = max(period.migration_rates[p][p2], self.params.min_M / N_A )
                    period.migration_rates[p][p2] = min(period.migration_rates[p][p2], self.params.max_M / N_A )
    ...
    def from_string(self, string):
        ...
        for p in periods:
            ...
            if len(params) == 1:
                self.add_period(
                    Period(0.0, [float(params[0][1:-1])], is_first_period=True))
                continue
            if params[0].endswith('%'):
                sizes = params[1][:-1].split(',')
                proportion = max(self.params.min_N, float(sizes[-2])) / self.periods[-1].get_sizes_of_populations()[-1]
                if float(sizes[-1]) == 0:
                    proportion = 1 - 1 / NA
                if float(sizes[-2]) == 0:
                    proportion = 1 / NA
                self.add_period(Split(
                    proportion, -1, self.periods[-1].get_sizes_of_populations()))
                continue
            sizes = ast.literal_eval('[' + params[1])
            self.add_period(Period(float(params[0]), sizes, ast.literal_eval(
                '[' + params[2]), None if len(params) < 4 or self.params.no_mig else ast.literal_eval(('[' + params[3]).replace('][', '], ['))))
    ...
    def construct_from_vector(self, vector, normalize=False):
        ...
        N_A is missed. It is equal to 1.0.
        Structure of model doesn't change.
        '''
        cur_index = 0
        vector = list(vector)
        for i, period in enumerate(self.periods):
            if period.is_first_period:
                ...
                period.update(self.periods[i - 1].get_sizes_of_populations(), self.params.min_N)
                cur_index += 1
            else:
...
#        #we can get that some values of params are already == ..._bound, we want to shift it a litle to perform optimization
#        little_shift = 1e-18
        ...
        for period in self.periods:
            ...
            if period.is_split_of_population:
                ...
                lower_bound.extend([self.params.min_N for _ in xrange(period.number_of_populations)])
                upper_bound.extend([self.params.max_N for _ in xrange(period.number_of_populations)])
        for period in self.periods:
    ...
    def run_local_search(self, name_of_search, filename):
        ...
        p0 = self.as_vector()[1:]
        verbose = self.params.ls_verbose if self.params.ls_verbose is not None else len(p0)
        func_kwargs = {'output_file':filename, 'lower_bound':lower_bound, 'upper_bound':upper_bound,
                'verbose':verbose, 'flush_delay':self.params.ls_flush_delay, 'epsilon':self.params.ls_epsilon,
                'gtol':self.params.ls_gtol, 'maxiter':self.params.ls_maxiter}
        ...
        if self.params.moments_scenario:
            p_opt = optimize_func(p0, self.params.input_data, self.moments_code, **func_kwargs)
        elif name_of_search == 'optimize_powell':
            # we need to run moments function with dadi's one, so we create extra function abd put pts inside
            func_ex = dadi.Numerics.make_extrap_log_func(self.dadi_code)
            def my_func(*args, **kwargs):
                kwargs['pts'] = self.params.dadi_pts
                return func_ex(*args, **kwargs)
            p_opt = optimize_func(p0, self.params.input_data, my_func, **func_kwargs)
        else:
            func_ex = dadi.Numerics.make_extrap_log_func(self.dadi_code)
            p_opt = optimize_func(p0, self.params.input_data, func_ex, self.params.dadi_pts, **func_kwargs)
    ...
    def has_changed(self):
        '''
        If the model has been changed, then old spectra and fitness function value aren't value.
        '''
        self.sfs = None
        self.fitness_func_value = None
        self.bic_score = None
    ...
    def get_number_of_params(self):
        return sum([p.number_of_parameters for p in self.periods]) - (1 if self.params.multinom else 0)

    def get_param_ids(self):
        '''
        Returns a list of pairs: (number of period, number of parameter of this period).
        '''
    ...
    def mutate_one(self, mutation_rate, index_and_sign=None):
        ...
        if self.params.distribution == 'uniform':    
            ...
            while change == 0:
                change = support.sample_from_truncated_normal(mutation_rate, self.params.std, 0.0, 1.0)
        ...
        if not self.periods[period_index].is_first_period and not self.periods[period_index].is_split_of_population and param_index == 0:
            self.check_time_and_correct_it()
            
        # if split we need to update sizes
        if self.periods[period_index].is_split_of_population:
            self.periods[period_index].update(
                self.periods[period_index - 1].get_sizes_of_populations(), self.params.min_N, N_A=self.get_N_A())

        # if we change sizes before split it should be not less than 2*min_N, and we need to update split
        if period_index + 1 != self.number_of_periods and self.periods[period_index+1].is_split_of_population and param_index == self.periods[period_index + 1].population_to_split + 1:
            self.periods[period_index].sizes_of_populations[param_index - 1] = max(
                    self.periods[period_index].sizes_of_populations[param_index - 1], 2 * self.params.min_N * self.get_N_A())
            self.periods[period_index+1].update(
                    self.periods[period_index].get_sizes_of_populations(), self.params.min_N, N_A=self.get_N_A())
        ...
        if not self.params.multinom and param_index == 0:
            ...
            for i, j in enumerate(self.get_structure()[:-1]):
                self.periods[i + j].update(self.periods[i+j-1].get_sizes_of_populations(), self.params.min_N, N_A=N_A)
        ...
        if period_index != (self.number_of_periods - 1):
            if self.periods[period_index + 1].is_split_of_population:
                self.periods[period_index + 1].update(
                    self.periods[period_index].get_sizes_of_populations(), self.params.min_N)
    ...
    def mutate(self, mutation_strength=None, mutation_rate=None, inds_and_signs=None):
        ...
        if inds_and_signs is None:
            number_of_params_to_change = max(1, np.random.binomial(n=len(self.get_param_ids()), p=mutation_strength))
        else:
    ...
    def increase_complexity(self, index=None):
        ...
        if index is not None:
            ...
            if self.periods[start_index_to_divide].is_first_period and len(p) > 1:
                p[0] = min(p[1:])
            p = np.array(p)
            p /= sum(p)
        ...
        if period_index_to_divide == 0:
            if total_time == 0:
                time = np.random.uniform(self.params.min_T, self.params.max_T) * self.get_N_A()
            else:
                time = total_time / self.number_of_periods
            self.periods.insert(1,
                                Period(
                                    time=time,
            ...
            self.periods[period_index_to_divide].time /= 2.0
            pops_sizes = []
            pops_exp = []
            for p in xrange(
                    self.periods[period_index_to_divide].number_of_populations):
                if self.periods[period_index_to_divide].growth_types[p] == 0:
                    pops_sizes.append(self.periods[period_index_to_divide]
                                      .get_sizes_of_populations()[p])
                    pops_exp.append(0)
                elif self.periods[period_index_to_divide].growth_types[p] == 1:
                    pops_sizes.append(
                        (self.periods[period_index_to_divide -
                                      1].get_sizes_of_populations()[p] + self.
                         periods[period_index_to_divide].get_sizes_of_populations()[p]
                         ) / 2)
                    pops_exp.append(1)
                else:
                    pops_sizes.append((self.periods[period_index_to_divide -
                                       ...
                                       self.periods[period_index_to_divide]
                                       .get_sizes_of_populations()[p])**(0.5))
                    pops_exp.append(2)
            self.periods.insert(
                period_index_to_divide,
                Period(
                    time=self.periods[period_index_to_divide].time,
                    sizes_of_populations=pops_sizes,
                    migration_rates=copy.deepcopy(
                        self.periods[period_index_to_divide].migration_rates),
        ...
        index_in_num_of_ch = sum([p.number_of_parameters for p in self.periods[:ind]]) - int(self.params.multinom)
    ...
    def normalize_by_Nref(self, N_ref=None):
        ...
        if N_ref == None:
            if self.params.moments_scenario:
                opt_scale = moments.Inference.optimal_sfs_scaling(self.get_sfs(),
                                                                  self.params.input_data)
            else:
                opt_scale = dadi.Inference.optimal_sfs_scaling(self.get_sfs(),
                                                               self.params.input_data)
            #checks for splits upper bounds:
            if self.params.split_1_lim is not None:
                opt_scale = min(
                    opt_scale, self.params.split_1_lim / sum([p.time for p in self.periods[self.split_1_pos:]]))
            if self.params.split_2_lim is not None:
                opt_scale = min(
                    opt_scale, self.params.split_2_lim / sum([p.time for p in self.periods[self.split_2_pos:]]))
            N_ref = opt_scale

        for period_number, period in enumerate(self.periods):
            if period.is_split_of_population:
                period.update(self.periods[period_number - 1].get_sizes_of_populations(), self.params.min_N)
                continue
    ...
    def check_time_and_correct_it(self):
        if self.params.split_1_lim is not None:
            sum_of_time = sum([p.time for p in self.periods[self.split_1_pos:]])
            if sum_of_time > self.params.split_1_lim:
                for p in self.periods[self.split_1_pos:]:
                    p.time *= self.params.split_1_lim / sum_of_time
                self.has_changed()
        if self.params.split_2_lim is not None:
            sum_of_time = sum([p.time for p in self.periods[self.split_2_pos:]])
    ...
    def generate_migration_rates(self, sizes_of_pops):
        ...
        sizes_of_pops regulate mig rates. Migration rate must be of order 1/N.
        '''
        N_A = self.get_N_A()
        ...
        if number_of_populations == 2:
            return [[
                None,
                random.choice([0, 1]) * np.random.uniform(self.params.min_M, min(self.params.max_M, 10.0 / sizes_of_pops[0])) / N_A
            ], [random.choice([0, 1]) * np.random.uniform(self.params.min_M, min(self.params.max_M, 10.0 / sizes_of_pops[1])) / N_A, None]]
        if number_of_populations == 3:
            return [[
                ...
                random.choice([0, 1]) * np.random.uniform(self.params.min_M, min(self.params.max_M, 10.0 / sizes_of_pops[1])) / N_A, None,
                random.choice([0, 1]) * np.random.uniform(self.params.min_M, min(self.params.max_M, 10.0 / sizes_of_pops[1])) / N_A
            ], [
                random.choice([0, 1]) * np.random.uniform(self.params.min_M, min(self.params.max_M, 10.0 / sizes_of_pops[2])) / N_A,
                random.choice([0, 1]) * np.random.uniform(self.params.min_M, min(self.params.max_M, 10.0 / sizes_of_pops[2])) / N_A, None
            ]]
    ...
    def get_fitness_func_value(self, data_sample=None):
        ...
        if self.fitness_func_value is None:
            ...
            self.bic_score = math.log(np.prod(
                np.array(self.params.ns))) * self.get_number_of_params() - 2 * log_likelihood
    ...
    def cross_with_other(self, other):
        ...
        take_from_other = np.array([random.random() for _ in xrange(self.get_number_of_params())]) > 0.5
        ...
        for i, period in enumerate(child.periods):
            if period.is_first_period:
                ...
                if take_from_other[cur_ind]:
                    period.get_sizes_of_populations()[0] = other.periods[i].get_sizes_of_populations()[0]
                cur_ind += 1
                N_A = child.get_N_A()
                continue
            ...
            if period.is_split_of_population:
                ...
                period.update(
                    child.periods[i - 1].get_sizes_of_populations(), self.params.min_N, N_A=N_A)
                cur_ind += 1
                continue
            ...
            for p in xrange(period.number_of_populations):
                if take_from_other[cur_ind]:
                    period.sizes_of_populations[p] = other.periods[i].sizes_of_populations[p]
                if take_from_other[cur_ind + period.number_of_populations]:
                    period.growth_types[p] = other.periods[i].growth_types[p]
                period.sizes_of_populations[p] = min(period.sizes_of_populations[p], self.params.max_N * N_A)
                period.sizes_of_populations[p] = max(period.sizes_of_populations[p], self.params.min_N * N_A)
                if i + 1 < self.number_of_periods and self.periods[i+1].is_split_of_population and p == self.periods[i+1].population_to_split:
                    period.sizes_of_populations[p] = max(period.sizes_of_populations[p], 2 * self.params.min_N * N_A)
                cur_ind += 1
            ...
            for p1 in xrange(period.number_of_populations):
                for p2 in xrange(period.number_of_populations):
                    if p1 == p2:
                        continue
                    if take_from_other[cur_ind]:
                        period.migration_rates[p1][p2] = other.periods[i].migration_rates[p1][p2]
                    period.migration_rates[p1][p2] = min(period.migration_rates[p1][p2], self.params.max_M / N_A)
                    period.migration_rates[p1][p2] = max(period.migration_rates[p1][p2], self.params.min_M / N_A)
                    cur_ind += 1
        
        child.number_of_changes = np.array(child.number_of_changes)
        child.number_of_changes[take_from_other] = other.number_of_changes[take_from_other]
            ...
            if period.is_first_period:
                ...
                for i in xrange(period.number_of_populations):
                    if period.growth_types[i] == 0:
                        growth_funcs.append(period.get_sizes_of_populations()[i])
                    elif period.growth_types[i] == 1:
                        growth_funcs.append(
                            _linear_growth_func(
                                self.periods[pos - 1].get_sizes_of_populations()[i],
                                period.get_sizes_of_populations()[i], period.time))
                    else:
                        growth_funcs.append(
                            _expon_growth_func(
                                self.periods[pos - 1].get_sizes_of_populations()[i],
                ...
                if period.number_of_populations == 1:
                    ...
                    phi = dadi.Integration.two_pops(
                        ...
                        nu1=growth_funcs[0],
                        nu2=growth_funcs[1],
                        m12=(0 if period.migration_rates == None else
                             period.migration_rates[0][1]),
                        m21=(0 if period.migration_rates == None else
                    ...
                    phi = dadi.Integration.three_pops(
                        ...
                        m12=0 if period.migration_rates == None else
                        period.migration_rates[0][1],
                        m13=0 if period.migration_rates == None else
                        period.migration_rates[0][2],
                        m21=0 if period.migration_rates == None else
                        period.migration_rates[1][0],
                        m23=0 if period.migration_rates == None else
                        period.migration_rates[1][2],
                        m31=0 if period.migration_rates == None else
                        period.migration_rates[2][0],
                        m32=0 if period.migration_rates == None else
        ...
        with open(filename, 'w') as output:
            ...
            for pos, period in enumerate(self.periods):
                if period.is_first_period:
                    ...
                    if period.number_of_populations == 2:
                        ...
                        output.write("\tbefore[-2] *= Ns[" + str(cur_index + extra) +
                                     "]\n")
                    ...
                    for i in xrange(period.number_of_populations):
                        if period.growth_types[i] == 0:
                            ...
                            growth_funcs.append(
                                _expon_growth_func_str("before[" + str(i) + "]",
                                                       "after[" + str(i) + "]",
                                                       "T"))
                    ...
                    if period.number_of_populations == 1:
                        ...
                        output.write("\tgrowth_func_2 = " + growth_funcs[1] +
                                     "\n")
                        output.write(
                            "\tphi = dadi.Integration.two_pops(phi, xx,  T=T, nu1=growth_func_1, nu2=growth_func_2, m12="
                            + ("0" if period.migration_rates == None else
                        ...
                        output.write("\tgrowth_func_3 = " + growth_funcs[2] +
                                     "\n")
                        output.write(
                            "\tphi = dadi.Integration.three_pops(phi, xx,  T=T, nu1=growth_func_1, nu2=growth_func_2, nu3=growth_func_3, m12="
                            + ("0" if period.migration_rates == None else
            ...
            if ext == "fs":
                ...
                output.write("data = dadi.Spectrum.from_data_dict(dd, pop_ids="
                             + str(self.params.pop_labels) + ", projections=" +
                             str(list(self.params.ns)) + ", polarized=" + str(self.params.input_data.folded) + ")\n")
            ...
            output.write("ll_model = dadi.Inference.ll(model, data)\n"
            ...
            if period.is_first_period:
                ...
                for p in xrange(period.number_of_populations):
                    if period.growth_types[p] == 0:
                        growth_funcs.append(
                            _sudden_growth_func(period.get_sizes_of_populations()[p]))
                    elif period.growth_types[p] == 1:
                        growth_funcs.append(
                            _linear_growth_func(
                                self.periods[pos - 1].get_sizes_of_populations()[p],
                                period.get_sizes_of_populations()[p], period.time))
                    else:
                        growth_funcs.append(
                            _expon_growth_func(
                                self.periods[pos - 1].get_sizes_of_populations()[p],
        ...
        with open(filename, 'w') as output:
            ...
            if ext == "fs":
                ...
                output.write("dd = moments.Misc.make_data_dict('" + os.path.
                             abspath(self.params.input_file) + "')\n")
                output.write(
                    "data = moments.Spectrum.from_data_dict(dd, pop_ids=" + str(
                        self.params.pop_labels) + ", projections=" + str(
                            list(self.params.ns)) + ", polarized=" + str(self.params.input_data.folded) + ")\n")
            output.write("\npopt = " + str(self.as_vector()) + "\n")
            output.write("ns = " + str(list(self.params.ns)) + "\n")
            output.write("model = generated_model(popt, ns)\n")
            output.write("ll_model = moments.Inference.ll(model, data)\n"
                         "ll_true = moments.Inference.ll(data, data)\n"
                         "print('Model log likelihood (LL(model, data)): {0}'.format(ll_model))\n")

            size_of_first_pop = self.get_N_A()
            self.normalize_by_Nref(1 / size_of_first_pop)
            output.write("#now we need to norm vector of params so that first value is 1:\n"
                         "popt_norm = " + str(self.as_vector()) + "\n")
            ...
            output.write("print('Drawing model to model_from_GADMA.png')\n"
                         ...
                         "\tpop_labels=" + str(self.params.pop_labels) + ",\n"
    ...
    def as_vector(self):
        '''
        Vector representaton of the model in order to put it in python code for DaDi.
        '''
        vector = []
        for period in self.periods:
            if period.is_first_period:
    ...
    def get_bounds_to_dadi(self):
        '''
        Bound for parameters of the demographic model in order to run DaDi optimization.
        '''
        vector_1 = []
        vector_2 = []
        max_size = 0
