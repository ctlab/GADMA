...
def run_genetic_algorithm(params_tuple):
    ...
    number = 'ID'
    try:
        write_func = lambda string: support.write_log(log_file, string,
...
def draw_bootstrap(logLL_models, output_dir, log_file):
    '''
    Calculate means, maxs, mins and so on for demographic models,
    print it to file and draw boxplots.
    ...
    if not options.options_storage.test:
        write_func("You can find bootstrap comparison of models in output directory.\n")
...
def print_best_solution_now(start_time, shared_dict,
    ...
    my_str = lambda x: support.float_representation(x, precision)
...
def main():
    try:
        ...
        params_filepath = os.path.join(options.options_storage.output_dir, 'params')
        options.options_storage.save(options.options_storage.output_dir)
        if not options.options_storage.test:
            support.write_log(log_file, "You can find all parameters of this run in:\t\t" + params_filepath + '\n')
            support.write_log(log_file, "All output is saved (without warnings and errors) in:\t" + os.path.join(options.options_storage.output_dir, 'GADMA.log\n'))
...
#        run_genetic_algorithm((1, log_file, None))
        ...
        pool = Pool(processes=options.options_storage.processes, initializer=worker_init)
        ...
        while True:
            try:
                multiple_results = pool_map.get(60 * options.options_storage.time_for_print)
                break
            # catch TimeoutError and get again
            except multiprocessing.TimeoutError as ex:
                min_counter += 1
                if min_counter < options.options_storage.time_for_bootstrap:
                    print_best_solution_now(start_time, shared_dict, log_file, precision)
                else:
                    min_counter = 0
