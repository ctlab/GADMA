...
class GlobalOptimizerAndLocalOptimizer(GlobalOptimizer, ConstrainedOptimizer):
    ...
    def optimize(self, f, variables, args=(), global_num_init=50,
        ...
        with open("./test_output.txt", "w") as output:
            output.write("HOHOHOHOHOHOHOHOHOHOHO")
            output.write("x_best[is_not_discrete]")
            output.write(f"{x_best}")
            output.write(f"{is_not_discrete}")
            output.write("local_result.x")
            output.write(f"{local_result.x}")
