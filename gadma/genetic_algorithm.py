...
class GA(object):
    ...
    def __init__(self, params, prefix=None, chromosomes_only=False, one_initial_model=None):
        '''
        ...
        params :    an object with parameters to work with
            ...
            frac_of_old_models : the fraction of models from the previous population in a new one
            frac_of_mutated_models : the fraction of mutated models in a new population
            frac_of_crossed_models : the fraction of crossed models in a new population
            mutation_rate : the rate to change one parameter of model in order to get mutatetd model from it
        ...
        self.number_of_random_models = params.size_of_population - self.number_of_old_models - \
            self.number_of_mutated_models - self.number_of_crossed_models
    ...
    def restore(self):
        def restore_from_cur_pop_of_models(list_of_str):
            for restore_str in list_of_str:
                self.models.append(Demographic_model(
                    self.params, restore_string=restore_str.strip().split('\t')[3]))
            if not (self.models[0].get_structure() <=
                    self.params.final_structure).all():
                raise RuntimeError("New final structure is less than current structure.")
        def restore_iteration_state(iter_out, size):
            ...
            if not iter_out[end_ind].startswith('Current mean mutation rate:'):
                raise RuntimeError("Expect 'Current mean mutation rate:' after current population during restoring.")
            self.cur_mutation_rate = float(iter_out[end_ind].split(':')[-1])
            
            if not iter_out[end_ind + 1].startswith('Current mean number of params to change:'):
                raise RuntimeError("Expect 'Current mean number of params to change:' after current population during restoring.")
            self.cur_mutation_strength = float(iter_out[end_ind + 1].split(':')[-1]) / self.models[0].get_number_of_params()
        ...
        if not os.path.isfile(os.path.join(self.params.resume_dir, self.prefix, "GADMA_GA.log")):
            return
        support.write_log(self.log_file, "GA number " + self.prefix + " find dir to restore")
        iter_out = []
        prev_iter_out = []
        self.models = []
        ...
        if len(iter_out) == 0 or not iter_out[0].startswith('Iteration #'):
            support.write_log(self.log_file, "GA number " + self.prefix + " find empty dir to restore. It would be run from random models.")
            return
        ...
        if len(iter_out) - 13 > size:
            if iter_out[-1].startswith("BEST") and iter_out[-2].startswith("Try to improve"):
            ...
            if iter_out[-1].startswith("Current mean number of params to change:"):
                # remove string
                iter_out.pop()
    ...
    def init_first_population_of_models(self):
        ...
        If self.params.restore_dir is not None, then population will be restored from previous run.
        '''
        ...
        support.write_to_file(self.log_file, "Best log likelihood:",
                              support.float_representation(-self.best_fitness_value(), self.ll_precision))
        support.write_to_file(self.log_file, "with BIC score:\t",
    ...
    def get_mutated_model(self, mutation_strength=None, mutation_rate=None, p=None):
        '''
        ...
        new_model_2.mutate(mutation_strength, mutation_rate, [(ind, -sgn) for ind, sgn in inds_and_signs])
    ...
    def select(self, size, print_iter=True):
        ...
        support.write_to_file(
            self.log_file, "Current population of models:\nN\tlogLL\t\t\tBIC\t\t\t\tmodel")
        for i, model in enumerate(self.models):
            support.write_to_file(self.log_file, i,
                                  support.float_representation(-model.get_fitness_func_value(), self.ll_precision), 
                                  support.float_representation(model.get_bic_score(), self.ll_precision), model)
        self.check_best_bic()
    ...
    def run_one_iteration(self):
        ...
        for i in xrange(self.number_of_mutated_models):
            model = self.get_mutated_model(mutation_strength=self.cur_mutation_strength,
                    mutation_rate=self.cur_mutation_rate, p=p)
        ...
        for i in xrange(self.number_of_random_models):
            new_models.append( self.get_random_model(structure=self.models[0].cur_structure))
        ...
        #try to make better mutated and crossed models, random are so by definition
        if not self.chromosomes_only and (self.without_changes == self.it_without_changes_to_stop_ga / 2 or self.is_stoped()):
        ...
        if abs(prev_value_of_fit - self.best_fitness_value()) < self.params.epsilon:
    ...
    def run_hill_climbing_of_best(self):
        ...
        while local_without_changes < self.params.hc_stop_iter:
            ...
            mut_rate = self.update_mutation_rate(self.params.hc_const_for_mut_rate, prev_value_of_fit < self.models[0])

            if flag:
                local_without_changes = 0
                support.write_to_file(
                    self.log_file, it,
                    support.float_representation(-self.models[0].get_fitness_func_value(), self.ll_precision), self.models[0])
                if not self.params.code_iter == 0 and self.cur_iteration % self.params.code_iter == 0:
                    self.models[0].dadi_code_to_file(
                        os.path.join(self.out_dir, "python_code", "dadi",
            ...
            if not self.chromosomes_only and (local_without_changes == self.params.hc_stop_iter / 2 or local_without_changes == self.params.hc_stop_iter):
    ...
    def run(self, shared_dict=None):
        ...
        if self.prefix is not None:
            ...
            if not self.params.draw_iter == 0:
                support.ensure_dir_existence(os.path.join(self.out_dir, "pictures"))
            if not self.params.code_iter == 0:
                support.ensure_dir_existence(os.path.join(self.out_dir, "python_code"))
        ...
        def run_one_ga_and_one_ls(): 
            ...
            support.write_to_file(
                self.log_file,
                "\nTry to improve best model (" + self.params.optimize_name + ")")
            if self.params.optimize_name != 'hill_climbing':
                self.models[0].run_local_search(self.params.optimize_name,
                        os.path.join(self.out_dir, self.params.optimize_name + "_" + str(self.cur_iteration) + "_out"))
                self.check_best_bic()
            else:
            ...
            if self.params.code_iter != 0 and self.cur_iteration % self.params.code_iter == 0:
                self.models[0].dadi_code_to_file(
                    os.path.join(self.out_dir, "python_code", "dadi",
                                 "iteration_" + str(self.cur_iteration) + "_after_local_search.py"))
                self.models[0].moments_code_to_file(
                    os.path.join(self.out_dir, "python_code", "moments",
                                 "iteration_" + str(self.cur_iteration) + "_after_local_search.py"))
            self.check_best_bic()
            ...
            if not self.params.draw_iter == 0 and self.cur_iteration % self.params.draw_iter == 0:
                self.best_model().draw(
                    ...
                    support.float_representation(-self.best_model().get_fitness_func_value(), self.ll_precision) + ", BIC: " +
                    support.float_representation(self.best_model().get_bic_score(), self.ll_precision))
        ...
        if shared_dict is not None:
            shared_dict[self.prefix] = (copy.deepcopy(self.models[0]), self.best_model_by_bic)
        ...
        if not self.chromosomes_only:
            while not (self.models[0].cur_structure == self.params.final_structure
                       ).all():
    ...
    def update_mutation_rate(self, flag):
        if not flag:
            # nothing changed
            self.cur_mutation_rate /= (self.params.const_for_mut_rate) ** (0.25)
        else:
            # became better
            self.cur_mutation_rate *= self.params.const_for_mut_rate
            self.cur_mutation_rate = min(self.cur_mutation_rate, self.max_mutation_rate)

    def update_mutation_strength(self, flag):
        if not flag or not self.models[0].info.endswith('m'):
            # nothing changed
            self.cur_mutation_strength /= (self.params.const_for_mut_strength) ** (0.25)
            self.cur_mutation_strength = max(self.cur_mutation_strength, 1.0/self.models[0].get_number_of_params())
        else:
            # became better
    ...
    def update_mutation_rate_and_strength(self, prev_value_of_fit):
        flag =( prev_value_of_fit - self.models[0].get_fitness_func_value()) > 1e-8
        self.update_mutation_rate(flag)
        self.update_mutation_strength(flag)
        
    def print_mutation_rate_and_strength(self):
        support.write_to_file(self.log_file,
                "Current mean mutation rate: " + support.float_representation(self.cur_mutation_rate, 5))
        n_params = self.models[0].get_number_of_params()
        support.write_to_file(self.log_file,
                "Current mean number of params to change: " + str(max(1, int(n_params * self.cur_mutation_strength))))
